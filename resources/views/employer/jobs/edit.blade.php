<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Job') }}</h2>
    </x-slot>

    <div class="py-10">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('employer.jobs.update', $job) }}" class="bg-white shadow-sm sm:rounded-lg">
                @csrf
                @method('PATCH')
                ...
                <div class="space-y-6 p-6">
                    <div>
                        ...
                        <select id="company_id" name="company_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            @foreach ($companies as $company)
                                <option value="{{ $company->id }}" @selected((int) old('company_id', $job->company_id) === $company->id)>{{ $company->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="7" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('description', $job->description) }}</textarea>
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <x-text-input id="experience_level" name="experience_level" type="text" class="mt-1 block w-full" :value="old('experience_level', $job->experience_level)" />
                        ...
                        <div>
                            ...
                            <select id="employment_type" name="employment_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @foreach (['full_time' => 'Full time', 'part_time' => 'Part time', 'contract' => 'Contract', 'internship' => 'Internship'] as $value => $label)
                                    <option value="{{ $value }}" @selected(old('employment_type', $job->employment_type->value) === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="workplace_type" name="workplace_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @foreach (['remote' => 'Remote', 'hybrid' => 'Hybrid', 'on_site' => 'On site'] as $value => $label)
                                    <option value="{{ $value }}" @selected(old('workplace_type', $job->workplace_type->value) === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid gap-6 sm:grid-cols-3">
                        <div>
                            ...
                            <x-text-input id="salary_min" name="salary_min" type="number" min="0" class="mt-1 block w-full" :value="old('salary_min', $job->salary_min)" />
                            <x-input-error class="mt-2" :messages="$errors->get('salary_min')" />
                        </div>
                        <div>
                            <x-input-label for="salary_max" :value="__('Salary max')" />
                            <x-text-input id="salary_max" name="salary_max" type="number" min="0" class="mt-1 block w-full" :value="old('salary_max', $job->salary_max)" />
                            <x-input-error class="mt-2" :messages="$errors->get('salary_max')" />
                        </div>
                        <div>
                            <x-input-label for="status" :value="__('Status')" />
                            <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                <option value="draft" @selected(old('status', $job->status->value) === 'draft')>Draft</option>
                                <option value="published" @selected(old('status', $job->status->value) === 'published')>Published</option>
                            </select>
